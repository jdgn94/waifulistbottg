...
import WaifuFavoriteLists, {
  WaifuFavoriteListCreationAttributes,
} from "../models/waifuFavoriteList";

const waifuFavoriteLists: WaifuFavoriteListCreationAttributes[] = [
  {
    waifuListId: 127,
    position: 4,
...
const insertWaifuFavoriteLists = async (t: Transaction) => {
  try {
    let waifuFavoriteListsToInsert: WaifuFavoriteListCreationAttributes[] = [];

    await Promise.all(
      waifuFavoriteLists.map(async (waifuFavoriteList) => {
        const waifuFavoriteListInserted = await WaifuFavoriteLists.findOne({
    ...
    await WaifuFavoriteLists.bulkCreate(waifuFavoriteListsToInsert, {
      transaction: t,
    });

    global.logger.info(
      `Inserted waifu favorite lists ${waifuFavoriteListsToInsert.length} news`
    );
    return;
  } catch (error) {
...
export { insertWaifuFavoriteLists };
