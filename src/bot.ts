...
bot.command("addfavorite", async (ctx) => {
  ...
  if (!waifu && !position)
    return ctx.reply(
      "Debes enviar el mensaje de la siguiente manera\n/addfavorite <numero de tu lista> <posición deseada>\nSolo puedes tener a 9 personajes en esta lista",
      { reply_to_message_id: message.message_id }
    );
...
bot.hashtag(["yaoiFanBoy", "fanBoy", "yaoi"], async (ctx) => {
  ...
  ctx.replyWithSticker(
    "CAACAgEAAxkAAIDi1-DKqj8D0CM0QzQ0W3Rkc7pBWVRAAIeAANL72IWgBwm30ZgSj0bBA"
...
bot.hashtag("conversationType", async (ctx) => {
  ctx.reply(`Conversacion de tipo: ${ctx.message?.chat.type}`, {
...
bot.hashtag("gay", async (ctx) => {
  ...
  //   : null;
  ctx.replyWithSticker(
    "CAACAgEAAxkBAAIDkF-DK0g2dWKYx-w716WQN6FvUM-SAAIjAANL72IWW0PmnHdw13QbBA"
    // { reply_to_message_id: messageId }
  );
  return await addCountInChat(ctx);
});
...
bot.hashtag(["F", "f"], async (ctx) => {
  ...
  //   ? ctx.message.reply_to_message.message_id
  //   : null;
  ctx.replyWithSticker(await randomSticker(stickers, 2), {
    // reply_to_message_id: messageId,
  });
  return await addCountInChat(ctx);
});
...
bot.hashtag(["police", "policia"], async (ctx) => {
  ...
  //   ? ctx.message.reply_to_message.message_id
  //   : null;
  ctx.replyWithSticker(await randomSticker(stickers, 1), {
    // reply_to_message_id: messageId,
  });
  return await addCountInChat(ctx);
});
...
bot.hashtag(["trap", "isATrap"], async (ctx) => {
  ...
  //   ? ctx.message.reply_to_message.message_id
  //   : null;
  ctx.replyWithSticker(await randomSticker(stickers, 3), {
    // reply_to_message_id: messageId,
  });
  return await addCountInChat(ctx);
});
...
// hears
bot.hears(
  ["lanzar una moneda", "lanzar moneda", "Lanzar una moneda", "Lanzar moneda"],
  async (ctx) => {
    const { message } = ctx;
    function throwCoin() {
    ...
    ctx.reply(
      `@${message.from.username} ha lanzado una moneda y salio ${throwCoin()}.`,
      { reply_to_message_id: message.message_id }
    );
    return await addCountInChat(ctx);
  }
