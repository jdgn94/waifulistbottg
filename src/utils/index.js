...
const searchList = async (userId, chatIs, messageId, page = 1) => {
  ...
  const extras = Telegraf.Extra.inReplyTo(messageId)
    ...
    .markup((m) =>
      m.inlineKeyboard(
        buttonsToList(m, parseInt(data.page), parseInt(data.totalPages))
      )
    );
...
const searchFavorite = async (userId, chatId, page = 1, ctx, username) => {
  const { status, data } = await axios.get(
    `/waifu_list/favorites?chatId=${chatId}&userId=${userId}&page=${page}`
  );
  switch (status) {
    case 200:
      return await sendAlbum(ctx, data.waifus, data.totalPages, page, username);
    case 201:
      return await sendMessage(ctx, `@${username}. ${data.message}`);
    default:
      return await sendMessage(
        ctx,
        `Ocurrio un error obteniendo tu listado @${username}`
      );
  }
};
...
const searchSpecial = async (userId, chatId, page = 1, ctx, username) => {
  const { status, data } = await axios.get(
    `/special_image/list?chatId=${chatId}&userId=${userId}&page=${page}`
  );
  switch (status) {
    case 200:
      return await sendAlbumSpecial(
        ctx,
        data.list,
        data.totalPages,
        page,
    ...
    case 205:
      return await sendMessage(
        ctx,
        `@${username}. No tienes waifus en esta pagina`
      );
    default:
      return await sendMessage(
        ctx,
...
const searchFranchise = async (franchiseNumber, page, messageId) => {
  ...
  const { status, data } = await axios.get(
    "/franchises/list?franchise_number=" + franchiseNumber + "&page=" + page
  );
  console.log(status, data);

  const { message, extras } = await messageFranchiseData(
    data,
    franchiseNumber,
    messageId
  );
...
const formatedUsers = async (ctx, users) => {
...
const detailsFav = async (ctx) => {
  ...
  if (from.username !== username) return;
  const { status, data } = await axios.get(
    `/waifu_list/favorites_details?userId=${callback_query.from.id}&chatId=${callback_query.message.chat.id}`
  );
...
const changePageSpecial = async (ctx) => {
  // funcion para cambiar la pagina en el mensaje del listado de imagenes especiales
  const { data, message, from } = ctx.update.callback_query;
  const username = message.text.split(",")[0].slice(1);
  if (from.username !== username) return;
  const buttons = message.reply_markup.inline_keyboard[0];
  ...
  let nextPage;
  if (data == "nextPageSpecial")
    nextPage = buttonSelected[0].text.split(" ")[1];
  else nextPage = buttonSelected[0].text.split(" ")[2];
...
const changePageFranchise = async (ctx) => {
  ...
  let nextPage;
  if (data == "nextPageFranchise")
    nextPage = buttonSelected[0].text.split(" ")[1];
  else nextPage = buttonSelected[0].text.split(" ")[2];
  const result = await searchFranchise(
    franchiseNumber,
    nextPage,
    message.message_id
  );
...
const trade = async (ctx, action) => {
  ...
  const dataTrade = await axios.get(
    "/waifu_list/trade_info?messageId=" + message.message_id
  );
  ...
  switch (status) {
    case 200:
      ...
      const emiterParams = {
        chatId: dataTrade.data.chatId,
        userId: dataTrade.data.user_emiter_id,
        waifuId: dataTrade.data.waifu_receptor_id,
        newWaifu: true,
      };
      ...
      if (dataEmiter)
        sendMessage(
          ctx,
          `@${dataTrade.data.user_emiter_name}, se ha agregado una nueva imagen a tu lista de favoritos`
        );
      const receptorParams = {
        chatId: dataTrade.data.chatId,
        userId: dataTrade.data.user_receptor_id,
        waifuId: dataTrade.data.waifu_emiter_id,
        newWaifu: true,
      };
      const dataReceptor = await addSpecial.addImageSpecialToList(
        receptorParams
      );
      if (dataReceptor)
        sendMessage(
          ctx,
          `@${dataTrade.data.user_receptor_name}, se ha agregado una nueva imagen a tu lista de favoritos`
        );
      return;
    case 201:
      ...
      return sendMessage(
        ctx,
        `El intercambio fue rechazado por ${message.from.username}`
      );
    case 202:
...
// INFO: botones
const buttonsToTrade = async (ctx, data) => {
  // botones para el intercambio de waifus
...
const buttonsToList = (m, page, totalPages) => {
...
const buttonsFavorites = (m, page, totalPages) => {
  // funcion para el pintado de los botones para el mensaje del listado
...
const buttonsSpecial = (m, page, totalPages) => {
  // funcion para el pintado de los botones para el mensaje del listado
  const buttonPrevius = m.callbackButton(
    `⬅️ Página ${page - 1}`,
    "previusPageSpecial"
  );
  const buttonNext = m.callbackButton(
    `Página ${page + 1} ➡️`,
    "nextPageSpecial"
  );
...
const waifusFormated = async (waifus, addCaption = true) => {
  return await waifus.map((waifu) => {
    return {
      media: {
        url: waifu.fav_image_url ? waifu.fav_image_url : waifu.image_url,
      },
      filename: waifu.fav_public_id ? waifu.fav_public_id : waifu.public_id,
      caption: addCaption
        ? `${waifu.position ? medalPosition(waifu.position) + ".- " : ""}${
            waifu.name
          } - ${waifu.franchise}`
        : "",
      type: "photo",
...
const sendSticker = async (ctx, stickers = [], personal = false) => {
  if (stickers.length == 0)
    return sendMessage(ctx, "Este sticker no esta definido todavia", {
      reply_to_message_id: ctx.message.message_id,
    });

  const messageId = personal
  ...
  await addCountInChat(ctx);
  const messageData = await ctx.replyWithSticker(sticker, {
...
const sendAnimationLink = async (ctx, links = []) => {
  if (links.length == 0)
    return sendMessage(ctx, "No hay animacion para este Hashtag", {
      reply_to_message_id: ctx.message.message_id,
    });
  ...
  await addCountInChat(ctx);
  const messageData = await ctx.replyWithAnimation(messageFormated, {
    reply_to_message_id: messageId,
  });
  return messageData;
};
...
const sendAlbum = async (
  ...
  const messageGalery = await ctx.replyWithMediaGroup(formated, {
    reply_to_message_id: messageId,
  });

  const extras = Telegraf.Extra.inReplyTo(messageGalery[0].message_id)
    .markdown()
    .markup((m) =>
      m.inlineKeyboard(
        buttonsFavorites(m, parseInt(page), parseInt(totalPages))
      )
    );
  ...
  ctx.reply(
    `@${username}, este es tu listado.\nPágina: ${page}/${totalPages}`,
    extras
  );
  return;
};

const sendAlbumSpecial = async (
  ctx,
  list,
  totalPages,
  page,
  ...
  const extras = Telegraf.Extra.inReplyTo(messageGalery[0].message_id)
    .markdown()
    .markup((m) =>
      m.inlineKeyboard(buttonsSpecial(m, parseInt(page), parseInt(totalPages)))
    );

  const username = ctx.message ? ctx.message.from.username : usernameTemp;
  ctx.reply(
    `@${username}, este es tu listado especial, aqui se muestran las waifus que tienen un fuerte vinculo.\nPágina: ${page}/${totalPages}`,
    extras
  );
  return;
};
...
const formatedName = async (list, page = 1, quantity = true) => {
  const formated = await list.map((item, index) =>
    quantity
      ? `${(page - 1) * 20 + index + 1}.- ${item.name} cantidad: ${
          item.quantity
        }.`
...
const messageFranchiseData = async (data, franchiseNumber, messageId) => {
  ...
  const extras = Telegraf.Extra.inReplyTo(messageId)
    ...
    .markup((m) =>
      m.inlineKeyboard(
        buttonsFranchiseList(m, parseInt(data.page), parseInt(data.totalPage))
      )
    );
  ...
  if (franchiseNumber == 0) {
    ...
    message =
      "Este es todo el listado de las franquicias que tienen waifus.\n\n" +
      formated.join("\n");
  } else {
    const formated = await formatedName(data.list, data.page, false);
    message = `Este es elistado de las waifus que pertenecen a la franquicia ${
      data.franchise.name
    }, hay un total de ${data.franchise.quantity}.\n\n${formated.join("\n")}`;
  }
